  ...
  async update(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      const data = scope === null
        ? await treinamentosService.update(req.params.id, record)
        : await treinamentosService.updateScoped(req.params.id, scope, record);
      sendSuccess(res, data);
    } catch (err: unknown) {
      try { handleValidationError(res, err); } catch { sendError(res, (err as Error).message, 500); }
    }
  ...
  async delete(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      if (scope === null) {
        ...
        await treinamentosService.deleteScoped(req.params.id, scope);
      }

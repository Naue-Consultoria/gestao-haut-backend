...
export class CaptacoesService {
  ...
  async updateByBroker(id: string, brokerId: string, record: { oportunidade?: string; exclusivo?: string; origem?: string; vgv?: number }) {
    const { data, error } = await supabaseAdmin
      .from('captacoes')
      .update(record)
      .eq('id', id)
  ...
  async updateByPartner(id: string, brokerIds: string[], record: { oportunidade?: string; exclusivo?: string; origem?: string; vgv?: number }) {
    const { data, error } = await supabaseAdmin
      .from('captacoes')
      .update(record)
      .eq('id', id)
